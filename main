...
# read the data
df = pandas.read_csv('C:\\Users\\moder\\Downloads\\Master Tech Sector Data Analysis 2 - Sheet1.csv')
print(df)
daysAhead = 1
...
def main():
	...
	ndxtdf = pandas.read_csv('C:\\Users\\moder\\Downloads\\VGHCX.csv')
	ndxtdf = ndxtdf.sort_values(by=['Date'], ascending=True)
	ndxtPrices = list(ndxtdf['Close'])

	# find when 2012 starts
	startOfTwelve = list(df[df['PERMNO'] == 1]['DATE']).index(20120103)
